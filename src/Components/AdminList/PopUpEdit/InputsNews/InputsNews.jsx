...
export default function InputsNews({ dataItem, imageLoad, setDataItem, setImageLoad }) {
    return (
        <>
            ...
            <input type='text' placeholder='Заголовок' value={dataItem.title} onChange={(e) => handleInputChange(e.target.value, 'title', setDataItem)} />
            <input type='number' placeholder="Лайки" value={dataItem.likes} onChange={(e) => handleInputChange(e.target.value, 'likes', setDataItem)} />
            <input type="text" placeholder="Дата створення" value={dataItem.date} onChange={(e) => handleInputChange(e.target.value, 'date', setDataItem)} />
